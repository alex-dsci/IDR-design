...
from idr_design.feature_calculators.sub_features.isoelectric.main import ACID_BASE_RES, BASIC_RES, KAS_ALL, PKA_N_TERM, PKA_C_TERM, charge_at_pH
import os
...
def _halley_pI(_counts: dict[str, int], converge_thres: float) -> float:
    ...
    charge: float
    halley_delta: float
    guess = _quick_guess(lambda pH: charge_at_pH(pH, N_basic, zip(counts, KAS_ALL)))

    with open(f"{path_to_this_file}/pI_output.txt", "w") as output_file:
        # Halley's method
        charge, halley_delta = _charge_and_halleys_delta(guess, N_basic, zip(counts, KAS_ALL))
...
def _scipy_suite_pI(_counts: dict[str, int], guess: bool = False, **kwargs) -> float:
    ...
    if guess:
        ...
        if kwargs["method"] == "halley":
            scipy_root = root_scalar(lambda pH: _calc_charge_w_halleys(pH, N_basic, zip(counts, KAS_ALL)), x0=x0, fprime=True).root
        else:
...
def _calc_charge_w_halleys(pH: float, num_basic_res: int, counts_and_KAs: Iterator[tuple[int, float]]) -> tuple[float, float, float]:
    deriv_0: float
    deriv_1: float
    deriv_2: float
...
def _charge_and_halleys_delta(pH: float, num_basic_res: int, counts_and_KAs: Iterator[tuple[int, float]]) -> tuple[float, float]:
    charge: float
    deriv_1: float
    deriv_2: float
    charge, deriv_1, deriv_2 = _calc_charge_w_halleys(pH, num_basic_res, counts_and_KAs)
